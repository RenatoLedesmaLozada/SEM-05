...
public class ProductoController {
	...
	public ResponseEntity<List<Producto>> buscarAvanzado(@RequestParam String keyword, @RequestParam Integer stockMin) {
		return ResponseEntity.ok(service.buscarPorFiltroAvanzadoEM(keyword, stockMin));
	}

	// Reto 2: Endpoint para alerta de productos con stock bajo un umbral
	@GetMapping("/stock-bajo")
	public ResponseEntity<List<Producto>> buscarProductosConStockBajo(@RequestParam Integer umbral) {
		return ResponseEntity.ok(service.buscarProductosConStockBajo(umbral));
	}
}